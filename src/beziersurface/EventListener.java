...
public class EventListener implements GLEventListener {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        //gl.glRotated(rotatex,1.0, 1.0, 1.0);
        ...
        if (KeyBoardInput.transformei && tenhoDesenho == true) { // TENHO TRANSFORMAÇÕES LOGO, TENHO QUE MOSTRAR 2 SUPERFICIES UMA ANTES E A OUTRA DEPOIS
            ...
            if (percorrerTrans - 1 >= 1) { // SEGUNDA TRANSFORMAÇÃO
                ...
                gl.glPushMatrix();
                gl.glViewport(0, 0, Main.getWindowWidth() / 2, Main.getWindowHeight()); // TELA DA ESQUERDA
                gl.glColor3d(1, 0, 0);         // COR DO TEXTO
                gl.glRasterPos3d(-0.4, 1.5,0); // POSIÇÃO X E Y QUE O TEXTO TERÁ
                ...
                gl.glLineWidth(3);
                gl.glColor3d(0, 1, 0); // EIXO X
                gl.glBegin(GL2.GL_LINES);
                gl.glVertex3d(-1, -1.9,0);
                gl.glVertex3d(Main.getWindowWidth(),-1.9,0);
                gl.glEnd();

                gl.glColor3d(0, 0, 1);
                gl.glBegin(GL2.GL_LINES); // LINHA DO EIXO Y DA TELA DA ESQUERDA
                gl.glVertex3d(-0.8, -1.9,0);
                gl.glVertex3d(-0.8, Main.getWindowHeight(),0);
                gl.glEnd();
                ...
                surperficieAntes.Draw(10.0); // DESENHA A CURVA
                gl.glPopMatrix();

                gl.glViewport(Main.getWindowWidth() / 2, 0, Main.getWindowWidth() / 2, Main.getWindowHeight()); // DA DIREITA                
                gl.glColor3d(1, 0, 0);
                gl.glRasterPos3d(-0.4, 1.5,0); // POSIÇÃO X E Y QUE O TEXTO TERÁ
    ...
    public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
        ...
        glu.gluPerspective(85, 1.0, 1.0, 90.0); //glu.gluPerspective(fovy, aspect, zNear, zFar);
